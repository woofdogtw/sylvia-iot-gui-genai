  ...
  <q-page padding>
        ...
        <q-card-section>
          <q-form ref="addFormRef" greedy>
            ...
            <q-select
              ...
              :label="t('core.application.unitId')"
              :rules="[ruleRequired]"
              dense outlined emit-value map-options class="q-mb-sm"
            />
            <q-input
              v-model="addForm.hostUri"
              :label="t('core.application.hostUri')"
              :rules="[ruleRequired, ruleHostUri]"
              dense outlined class="q-mb-sm"
            />
            <q-input
              v-model="addForm.name"
              :label="t('core.application.name')"
              dense outlined class="q-mb-sm"
            />
            <q-input
              v-if="isAmqp(addForm.hostUri)"
              v-model="addForm.ttl"
              :label="t('core.application.ttl')"
              :rules="[ruleNonNeg]"
              dense outlined class="q-mb-sm"
            />
            <q-input
              ...
              :label="t('core.application.length')"
              :rules="[ruleNonNeg]"
              dense outlined class="q-mb-sm"
            />
            <q-input
              v-model="addForm.info"
              :label="t('core.application.info')"
              :rules="[ruleJson]"
    ...
    <q-dialog v-model="showPassword" @keyup.escape="showPassword = false">
      <q-card style="min-width: 400px">
        <q-card-section>
          <div class="text-h6">{{ t('core.common.password') }}</div>
        </q-card-section>
        <q-card-section>
          <q-input
        ...
        <q-card-actions align="right">
          <q-btn flat :label="t('core.common.close')" @click="showPassword = false" />
        </q-card-actions>
      </q-card>
    </q-dialog>
        ...
        <q-card-section>
          <q-form ref="editFormRef" greedy>
            ...
            <q-input
              ...
              :rules="hostUriChanged ? [ruleRequired] : []"
              type="password" dense outlined class="q-mb-sm"
            />
            <q-input
    ...
    <q-dialog v-model="showStats" @hide="closeStatsDialog">
      <q-card style="min-width: 500px">
        <q-card-section>
          <div class="text-h6">{{ t('core.common.stats') }} - {{ statsItem.code }}</div>
        </q-card-section>
        <q-card-section>
          <q-markup-table dense flat>
            ...
            <tbody>
              <tr v-for="q in ['uldata', 'dldataResp', 'dldataResult']" :key="q">
                <td>{{ t(`core.application.queue${q.charAt(0).toUpperCase() + q.slice(1)}`) }}</td>
                <td>{{ statsData[q]?.consumers ?? '-' }}</td>
                <td>{{ statsData[q]?.messages ?? '-' }}</td>
                <td>{{ statsData[q]?.publishRate ?? '-' }}</td>
                <td>{{ statsData[q]?.deliveryRate ?? '-' }}</td>
              </tr>
            </tbody>
          </q-markup-table>
        </q-card-section>
        <q-card-actions align="right">
          <q-btn flat :label="t('core.common.close')" @click="closeStatsDialog" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="showSend" @keyup.enter="submitSend" @keyup.escape="showSend = false">
      <q-card style="min-width: 450px">
        <q-card-section>
          <div class="text-h6">{{ t('core.common.sendData') }}</div>
        </q-card-section>
        <q-card-section>
          <q-form ref="sendFormRef" greedy>
            <q-select
              v-model="sendForm.deviceId"
              :options="deviceOptions"
              :label="t('core.application.deviceId')"
              :rules="[ruleRequired]"
              dense outlined emit-value map-options class="q-mb-sm"
            />
            <q-select
              v-model="sendForm.payloadType"
              :options="payloadTypeOptions"
              :label="t('core.common.payloadType')"
              dense outlined emit-value map-options class="q-mb-sm"
            />
            <q-input
              v-model="sendForm.payload"
              :label="t('core.common.payload')"
              :rules="sendForm.payloadType === 'hex' ? [ruleHex] : []"
              dense outlined class="q-mb-sm"
            />
          </q-form>
        </q-card-section>
        ...
        <q-card-section>
          <q-list dense>
            <q-item><q-item-section>
              <q-item-label caption>{{ t('core.application.code') }}</q-item-label>
              <q-item-label>{{ detailItem.code }}</q-item-label>
            </q-item-section></q-item>
            <q-item><q-item-section>
              <q-item-label caption>{{ t('core.application.name') }}</q-item-label>
              <q-item-label>{{ detailItem.name || '-' }}</q-item-label>
            </q-item-section></q-item>
            <q-item><q-item-section>
              <q-item-label caption>{{ t('core.application.unitId') }}</q-item-label>
              <q-item-label>{{ detailItem.unitId || '-' }}</q-item-label>
            </q-item-section></q-item>
            <q-item><q-item-section>
              <q-item-label caption>{{ t('core.application.hostUri') }}</q-item-label>
              <q-item-label>{{ detailItem.hostUri }}</q-item-label>
            </q-item-section></q-item>
            <q-item><q-item-section>
              <q-item-label caption>{{ t('core.application.protocol') }}</q-item-label>
              <q-item-label>{{ extractProtocol(detailItem.hostUri) }}</q-item-label>
            </q-item-section></q-item>
            <q-item><q-item-section>
              <q-item-label caption>{{ t('core.application.info') }}</q-item-label>
    ...
    <q-dialog v-model="showDelete" @keyup.enter="submitDelete" @keyup.escape="showDelete = false">
      <q-card>
        ...
        <q-card-section>
          {{ t('core.common.deleteConfirm', { name: deleteItem.code }) }}
        </q-card-section>
        <q-card-actions align="right">
...
import { applicationApi, unitApi, deviceApi } from '../api/index.js'
import {
...
const columns = computed(() => [
  { name: 'code', label: t('core.application.code'), field: 'code', align: 'left', sortable: true },
  { name: 'name', label: t('core.application.name'), field: 'name', align: 'left', sortable: true },
  { name: 'unitId', label: t('core.application.unitId'), field: 'unitCode', align: 'left' },
  { name: 'protocol', label: t('core.application.protocol'), field: 'protocol', align: 'left' },
...
const {
  ...
  apiList: applicationApi.list,
  apiCount: applicationApi.count,
  mapRow: (row) => ({
    ...row,
    unitCode: row.unitCode || row.unitId || '-',
...
function isAmqp(uri) {
  return uri && (uri.startsWith('amqp://') || uri.startsWith('amqps://'))
}
...
const hostUriChanged = computed(() => {
  return editingApp.value && editForm.value.hostUri !== editingApp.value.hostUri
})
...
async function submitEdit() {
  ...
  try {
    const data = {
      hostUri: trimValue(editForm.value.hostUri),
    ...
    await applicationApi.update(editingApp.value.applicationId, data)
...
async function fetchStats() {
  try {
    const res = await applicationApi.stats(statsItem.value.applicationId)
...
async function openSendDialog(row) {
  ...
  showSend.value = true
  // Load devices for this application's unit
  try {
    const params = { limit: 0 }
    if (row.unitId) params.unit = row.unitId
    const res = await deviceApi.list(params)
    deviceOptions.value = (res.data?.data || []).map((d) => ({
      label: d.networkAddr,
      value: d.deviceId,
    }))
  } catch { deviceOptions.value = [] }
  sendForm.value._applicationId = row.applicationId
}
...
async function submitSend() {
  ...
  try {
    ...
    if (sendForm.value.payloadType === 'text') {
      payload = textToHex(payload)
    }
    await applicationApi.sendDlData(sendForm.value._applicationId, {
      deviceId: sendForm.value.deviceId,
      payload,
    })
...
async function submitDelete() {
  try {
    await applicationApi.delete(deleteItem.value.applicationId)
...
async function onExportCsv() {
  try {
    await exportCsv('/api/v1/application/list', 'applications.csv', filterParams)
  } catch (err) { notifyApiError(err, t) }
}
