  ...
  <q-page padding>
      ...
      <q-card style="min-width: 450px">
        ...
        <q-card-section>
          <q-form ref="addFormRef" greedy>
            ...
            <q-select v-model="addForm.applicationId" :options="addAppOptions" :label="t('core.networkRoute.applicationCode')" :rules="[ruleRequired]" dense outlined emit-value map-options class="q-mb-sm" />
            <q-select v-model="addForm.networkId" :options="addNetworkOptions" :label="t('core.networkRoute.networkCode')" :rules="[ruleRequired]" dense outlined emit-value map-options class="q-mb-sm" />
          </q-form>
        </q-card-section>
    ...
    <q-dialog v-model="showDetail" @keyup.escape="showDetail = false">
      <q-card style="min-width: 450px">
        ...
        <q-card-section>
          <q-list dense>
            <q-item><q-item-section><q-item-label caption>{{ t('core.networkRoute.applicationCode') }}</q-item-label><q-item-label>{{ detailItem.applicationCode }}</q-item-label></q-item-section></q-item>
            <q-item><q-item-section><q-item-label caption>{{ t('core.networkRoute.networkCode') }}</q-item-label><q-item-label>{{ detailItem.networkCode }}</q-item-label></q-item-section></q-item>
    ...
    <q-dialog v-model="showDelete" @keyup.enter="submitDelete" @keyup.escape="showDelete = false">
      <q-card>
        ...
        <q-card-section>{{ t('core.common.deleteConfirm', { name: `${deleteItem.applicationCode} → ${deleteItem.networkCode}` }) }}</q-card-section>
        <q-card-actions align="right">
...
import { networkRouteApi, unitApi, applicationApi, networkApi } from '../api/index.js'
...
const columns = computed(() => [
  { name: 'applicationCode', label: t('core.networkRoute.applicationCode'), field: 'applicationCode', align: 'left', sortable: true },
  { name: 'networkCode', label: t('core.networkRoute.networkCode'), field: 'networkCode', align: 'left', sortable: true },
  { name: 'createdAt', label: t('core.common.created'), field: 'createdAtStr', align: 'left', sortable: true },
  { name: 'actions', label: '', field: 'actions', align: 'right' },
])

const visibleColumns = computed(() => {
  const cols = ['applicationCode', 'networkCode', 'actions']
...
const {
  ...
  apiList: networkRouteApi.list,
  apiCount: networkRouteApi.count,
...
const addForm = ref({ unitId: '', applicationId: '', networkId: '' })
const addAppOptions = ref([])
const addNetworkOptions = ref([])

function openAddDialog() {
  const preUnit = selectedUnit.value || ''
  addForm.value = { unitId: preUnit, applicationId: '', networkId: '' }
  addAppOptions.value = []; addNetworkOptions.value = []
  showAdd.value = true
...
async function onAddUnitChange(unitId) {
  try {
    const [appRes, netRes] = await Promise.all([
      applicationApi.list({ unit: unitId, limit: 0 }),
      networkApi.list({ unit: unitId, limit: 0 }),
    ])
    addAppOptions.value = (appRes.data?.data || []).map((a) => ({ label: a.code, value: a.applicationId }))
    addNetworkOptions.value = (netRes.data?.data || []).map((n) => ({ label: n.code, value: n.networkId }))
  } catch { addAppOptions.value = []; addNetworkOptions.value = [] }
}
...
async function submitDelete() {
  try { await networkRouteApi.delete(deleteItem.value.routeId); notifySuccess(t('core.common.deleteSuccess')); showDelete.value = false; refresh() }
  catch (err) { notifyApiError(err, t) }
}

async function onExportCsv() {
  try { await exportCsv('/api/v1/network-route/list', 'network-routes.csv', filterParams) }
  catch (err) { notifyApiError(err, t) }
}
