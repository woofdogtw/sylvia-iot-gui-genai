      ...
      <q-card style="min-width: 500px">
        ...
        <q-card-section>
          <q-list dense>
            ...
            <q-item><q-item-section><q-item-label caption>{{ t('data.common.pubTime') }}</q-item-label><q-item-label>{{ formatTime(detailItem.pub) }}</q-item-label></q-item-section></q-item>
            <q-item><q-item-section><q-item-label caption>{{ t('data.common.respTime') }}</q-item-label><q-item-label>{{ formatTime(detailItem.resp) }}</q-item-label></q-item-section></q-item>
            <q-item><q-item-section><q-item-label caption>{{ t('data.common.status') }}</q-item-label><q-item-label><q-badge :color="statusColor(detailItem.status)">{{ detailItem.status }}</q-badge></q-item-label></q-item-section></q-item>
...
function statusLabel(val) {
  ...
  if (val === 0) return t('data.status.success')
  if (val < 0) return t('data.status.processing')
  return t('data.status.error')
}
...
const columns = computed(() => [
  ...
  { name: 'pub', label: t('data.common.pubTime'), field: 'pubStr', align: 'left', sortable: true },
  { name: 'resp', label: t('data.common.respTime'), field: 'respStr', align: 'left', sortable: true },
...
const {
  ...
  apiList: networkDlDataApi.list,
  apiCount: networkDlDataApi.count,
  mapRow: (row) => ({
    ...row,
    procStr: formatTime(row.proc),
    pubStr: formatTime(row.pub),
    respStr: formatTime(row.resp),
